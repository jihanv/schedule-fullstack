...
function SectionLabel({ children }: { children: React.ReactNode }) {
    return <label className="mb-2 block text-sm font-semibold text-slate-900 dark:text-slate-100">{children}</label>;
}
...
function OutputPanel({ label, value }: OutputPanelProps) {
    return (
        <Card>
            ...
            <pre className="min-h-70 overflow-auto rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-900 whitespace-pre-wrap wrap-break-word dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100">
                {value}
            </pre>
        </Card>
    );
