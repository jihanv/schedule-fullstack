...
export default function WeeklyTables() {
  ...
  const meetingCount = React.useMemo(() => {
    ...
    for (const dl of deletedLessons) {
      deletedSet.add(`${dl.dateKey}|${dl.period}`);
    }
    ...
    for (const wk of weeks) {
      for (const d of wk.days) {
        ...
        for (const p of PERIODS) {
          ...
          // 2) Otherwise weekly template schedule (but ignore deleted slots)
        ...
        {weeks.map((week, wIdx) => (
          <div key={wIdx} className="rounded-2xl border bg-card p-4">
            <div className="mb-2 text-sm text-muted-foreground">
              {t("weekSubtitle", {
                date: format.dateTime(week.start, { dateStyle: "medium" }),
              })}
            </div>
                  ...
                  {PERIODS.map((p) => (
                    <tr key={p}>
                      ...
                      {week.days.map((d, i) => {
                        ...
                        const isDeleted = isDeletedLesson(cellDateKey, p);

                        const displaySection =
                          manual?.section ?? (isDeleted ? undefined : assigned);
                        ...
                        const classNum = meetingCount.get(
                          `${cellDateKey}|${p}`,
                        );
                        ...
                        const content = hol ? (
                          ...
                          <>
                            ...
                            <div
                              className={`text-xs leading-4 ${displaySection ? "font-semibold" : "text-muted-foreground"}`}
                            >
                              {displaySection ?? "—"}
                            </div>

                            <div
                              className={`text-xs leading-4 min-h-4 ${displaySection ? "opacity-100" : "text-muted-foreground"}`}
                            >
                              {displaySection
